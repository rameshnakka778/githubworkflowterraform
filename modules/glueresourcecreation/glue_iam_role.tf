...
resource "aws_iam_role" "js_dpp1_glue1" {
  ...
  assume_role_policy   = file("${path.module}/policies/grada_product_nonprd_glue_service_role_test.json")
}
...
resource "aws_iam_role_policy" "js_dpp1_glue1_glue_policy" {
  ...
  policy =file("${path.module}/policies/CloudWatchFullAccesspolicy.json.tpl")
}
...
resource "aws_iam_role_policy" "js_dpp1_glue1_policy1" {
  ...
  role = aws_iam_role.js_dpp1_glue1.id
  policy =file("${path.module}/policies/AwsGlueServicerolepolicy.json.tpl")
}

resource "aws_iam_role_policy" "js_dpp1_glue1_policy2" {
  name = "ramesh-${var.service_name}-${var.environment}-dpp_s3_read_access"
  role = aws_iam_role.js_dpp1_glue1.id
  policy =file("${path.module}/policies/grada_product_nonprod_s3_read_access.json.tpl")
}
