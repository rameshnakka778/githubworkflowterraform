...
resource "aws_iam_role" "js_dpp1_glue1" {
  ...
  assume_role_policy   = file("${path.root}/grada_product_nonprd_glue_service_role_test.json")
}
...
resource "aws_iam_role_policy" "js_dpp1_glue1_glue_policy" {
  ...
  policy =file("${path.root}/CloudWatchFullAccesspolicy.json.tpl")
}
